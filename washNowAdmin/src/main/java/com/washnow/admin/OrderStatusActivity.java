...
import com.washnow.admin.async.GetOrderTask;
import com.washnow.admin.async.OrderUpdateTask;
	...
	public void initLayout(){
        ...
        if(diff==0){
          ...
          if(order.getTotal()!=null)
               tvPrice.setText(order.getTotal() +" QAR");
          else  {
        	  tvPrice.setText("0 QAR");
        	  order.setTotal("0");
          }
...
//			Log.e("products " +cost,products);
		...
		//		new ProductsUpdateTask(context).execute(new String[]{order.getId(),products});
				order.setTotal(String.valueOf(cost));
				dataUpdated=true;
